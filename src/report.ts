...
<head>
  ...
  <style>
    ...
    * { box-sizing: border-box; }
    ...
    .global-topbar { position:sticky; top:0; z-index:50; display:grid; grid-template-columns:240px minmax(0,1fr) auto; align-items:center; gap:16px; min-height:64px; padding:8px 22px; color:var(--color-on-accent); background:var(--color-hero-end); box-shadow:0 2px 12px var(--color-shadow); }
    .global-brand { display:flex; align-items:center; gap:10px; color:var(--color-on-accent); font-size:19px; font-weight:900; text-decoration:none; }
    .global-brand-mark { display:grid; width:40px; height:40px; place-items:center; border-radius:50%; color:var(--color-on-accent); background:var(--gradient-brand); }
    .global-topbar-copy { color:var(--color-hero-copy); font-size:12px; font-weight:700; }
    .global-topbar-actions { display:flex; align-items:center; justify-content:flex-end; gap:8px; }
    .global-topbar-actions a { display:inline-flex; align-items:center; min-height:38px; padding:7px 12px; border-radius:8px; color:var(--color-on-accent); background:var(--gradient-brand); font-size:11px; font-weight:850; text-decoration:none; }
    .dashboard-app-shell { display:grid; grid-template-columns:240px minmax(0,1fr); min-height:calc(100vh - 64px); }
    .global-sidebar { position:sticky; top:64px; align-self:start; height:calc(100vh - 64px); overflow-y:auto; border-right:1px solid var(--color-line); background:var(--color-panel); }
    .global-sidebar nav { display:grid; padding:8px 0; }
    .global-sidebar-label { padding:12px 16px 7px; color:var(--color-muted); font-size:10px; font-weight:850; letter-spacing:.09em; text-transform:uppercase; }
    .global-sidebar button,.global-sidebar a { display:grid; grid-template-columns:38px minmax(0,1fr); align-items:center; width:100%; min-height:48px; padding:8px 16px; color:var(--color-ink); border:0; border-radius:0; background:var(--color-transparent); font-size:13px; font-weight:700; text-align:left; text-decoration:none; cursor:pointer; }
    .global-sidebar button:hover,.global-sidebar a:hover { background:var(--color-panel-subtle); }
    .global-sidebar button.active { color:var(--color-on-accent); background:var(--gradient-brand); }
    .global-sidebar-icon { color:var(--color-muted); font-size:17px; text-align:center; }
    .global-sidebar button.active .global-sidebar-icon { color:var(--color-on-accent); }
    .global-sidebar-separator { height:1px; margin:8px 0; background:var(--color-line); }
    ...
    @media (max-width: 760px) {
      ...
      .global-brand span:last-child,.global-topbar-copy { display:none; }
      .global-brand-mark { width:36px; height:36px; }
      .dashboard-app-shell { display:block; min-height:0; }
      .global-sidebar { position:sticky; top:0; z-index:40; width:100%; height:auto; overflow-x:auto; overflow-y:hidden; border-right:0; border-bottom:1px solid var(--color-line); }
      .global-sidebar nav { display:flex; width:max-content; padding:0; }
      .global-sidebar-label { display:flex; align-items:center; padding:8px 11px; white-space:nowrap; }
      .global-sidebar button,.global-sidebar a { display:flex; width:auto; min-height:48px; padding:8px 11px; white-space:nowrap; }
      .global-sidebar-icon { width:24px; }
      .global-sidebar-separator { width:1px; height:32px; margin:8px 2px; flex:0 0 auto; }
    }
...
<body>
  ...
  <script>
    ...
    const uiCopy = {
      ...
      mainMenu: { tr: "Ana menü", en: "Main menu" },
      ...
      steamAnalytics: { tr: "Steam Analitiği", en: "Steam Analytics" },
      gamesList: { tr: "Oyunlar Listesi", en: "Games List" },
      publishersList: { tr: "Yayıncılar Listesi", en: "Publishers List" },
      genresTags: { tr: "Türler ve Etiketler", en: "Genres and Tags" },
